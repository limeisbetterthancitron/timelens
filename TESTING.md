...
## 2. Live-world restoration

Exit must restore what is really there now, not a cached snapshot from when the view opened.

- [ ] A opens a view
- [ ] B changes one of the blocks inside A's view
- [ ] A runs `/timelens exit`
- [ ] A sees B's **new** block, not the state from when A started
...
Note when measuring: with nobody online, Paper unloads chunks immediately and TimeLens correctly
skips positions in unloaded chunks — a harness must hold plugin chunk tickets or it will measure
nothing at all and appear to pass.
